...
class Settings(BaseSettings):
    ...
    CHROMA_PERSIST_DIR: str = Field(default="./data/chroma_db", description="Directory for ChromaDB persistence")
    CHROMA_COLLECTION: str = Field(default="rag_docs", description="ChromaDB collection name")

    # ── Document Loading ───────────────────────────────────────────────────────
    DOCUMENTS_DIR: str = Field(default="./data/documents", description="Directory where user places PDFs/DOCX/TXT files")
